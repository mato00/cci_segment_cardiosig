...
def hs_process(data):
    # origin
    # data = downsample(data, sr, 800)
    
    data_o = local_wiener(data, mysize=21, local_size=100, fs=800)
    data_o = preprocessing.scale(data_o)
    """
    data_s = butter_bandpass_filter(data_o, lowcut=30, highcut=60, fs=1000, order=5)
    data_w = wavelet_envelop(data_o, 'rbio3.9', level_=6, keep_level_=3)

    data_o = temporal_norm(data_o)
    data_s = temporal_norm(data_s)
    data_w = temporal_norm(data_w)

    data_o = np.expand_dims(data_o, -1)
    data_s = np.expand_dims(data_s, -1)
    data_w = np.expand_dims(data_w, -1)

    data_test = np.concatenate((data_o, data_s, data_w), axis=-1)
    """
    data_test = np.expand_dims(data_o, -1)

    return data_test

def hs_process_batch(data):
    hss = []
    for single_hs in data:
        # single_hs = downsample(single_hs, sr, 800)

        data_o = local_wiener(single_hs, mysize=21, local_size=100, fs=800)
        data_o = preprocessing.scale(data_o)
        """
        data_s = butter_bandpass_filter(data_o, lowcut=30, highcut=60, fs=1000, order=5)
        data_w = wavelet_envelop(data_o, 'rbio3.9', level_=6, keep_level_=3)
