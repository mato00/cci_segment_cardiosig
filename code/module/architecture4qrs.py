...
from .densenet import DenseNet_qrs
...
def mb_encoder():
    ...
    return Model(x_in, feat, name='mb_encoder')

def densenet_encoder():
    dense_en = DenseNet_qrs(num_init_features=16, growth_rate=4, block_layers=[3, 5, 8], compression_rate=0.5)
...
def crnn_encoder():
    ...
    feat_3 = layers.Conv1D(filters=32,
                            ...
                            padding='same',
                            activation=None)(feat_3)
    feat_3 = layers.BatchNormalization()(feat_3)
    feat_3 = activations.relu(feat_3)
    feat_3 = layers.MaxPool1D()(feat_3)
    feat_3 = layers.Conv1D(filters=64,
                            kernel_size=5,
    ...
    feat = layers.Conv1D(filters=192,
                         ...
                         activation=None)(feat)
    ## LSTM
    feat = layers.LSTM(192, return_sequences=True, return_state=False)(feat)
...
def decoder():
    x_in = Input(shape=(625, 192))
    # feat = layers.Dense(96, activation='relu')(x_in)
    feat = layers.Dense(48, activation='relu')(x_in)
    # feat = layers.Dense(24, activation='relu')(feat)
    logits = layers.Dense(1, activation='sigmoid')(feat)
